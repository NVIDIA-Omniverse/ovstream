...
## Python: `ovstream.CudashmClient`

The minimal pattern:

```python
client = ovstream.CudashmClient("my-stream")
try:
...
`wait_frame(timeout_ms=...)` blocks up to the timeout, returning `None` on timeout or when no new frame has arrived. `is_producer_alive()` flips to `False` within ~100 ms of producer exit. The same reverse-channel `send_message` / `send_input_event` / `send_unicode` / `on_message` API as `ShmClient` is available — the producer's regular callbacks fire for CUDASHM events too.
...
## C / C++: `<ovstream/ovstream_cudashm_client.h>`

The C API surface (sibling of `ovstream_shm_client_*`):

- `ovstream_cudashm_client_create(name, &client)` — attach to a named producer. Imports every IPC handle into the calling thread's active CUDA context.
- `ovstream_cudashm_client_wait_frame(client, timeoutMs, &frame)` — block for a frame; populates `frame.device_ptr` with the imported device pointer for the latest slot.
- `ovstream_cudashm_client_is_producer_alive(client, &alive)` — watchdog.
- `ovstream_cudashm_client_destroy(client)` — detach. Closes every imported IPC handle.
- `ovstream_cudashm_client_send_input_event(client, &event)` — see [Reverse channel](#reverse-channel-driving-the-producer-from-the-consumer).
- `ovstream_cudashm_client_send_message(client, message)` — see [Reverse channel](#reverse-channel-driving-the-producer-from-the-consumer).
- `ovstream_cudashm_client_send_unicode(client, text)` — see [Reverse channel](#reverse-channel-driving-the-producer-from-the-consumer).
- `ovstream_cudashm_client_set_message_callback(client, cb, user_data)` — see [Reverse channel](#reverse-channel-driving-the-producer-from-the-consumer).

Useful when writing a native simulator, a GPU post-processing pipeline, or any C/C++ consumer that already has a CUDA context and wants pixels delivered as device pointers. The slim `ovstream_cudashm_client` shared library links the CUDA runtime (needed for `cudaIpcOpenMemHandle` / `cudaIpcCloseMemHandle`) but does **not** depend on StreamSDK or GStreamer.
...
C: see the corresponding `ovstream_cudashm_client_send_*` entries above. Constraints (16 MiB control-channel line limit, no embedded `\n` / `\r`) match SHM.
...
- If the producer is killed (clean exit or hard kill), `wait_frame` returns `None` and `is_producer_alive` flips. Consumer should detect that and exit cleanly.
...
| Python | C |
|--------|---|
| `ovstream.CudashmClient(stream_name)` | `ovstream_cudashm_client_create(name, &client)` |
| `client.wait_frame(timeout_ms=N)` | `ovstream_cudashm_client_wait_frame(client, N, &frame)` |
| `frame.device_ptr` / `frame.pitch_bytes` / `frame.width` / `frame.height` / `frame.slot_index` | (same fields on the C `ovstream_cudashm_frame_t`) |
| `client.is_producer_alive()` | `ovstream_cudashm_client_is_producer_alive(client, &alive)` |
| `client.producer_device` | `ovstream_cudashm_client_get_producer_device(client, &device)` |
| `client.close()` / context manager exit | `ovstream_cudashm_client_destroy(client)` |
...
- **Producer and consumer must be on the same GPU.** CUDASHM IPC handles are tied to a single CUDA device. The producer allocates its ring on `config.cuda_device` (or, when that's `-1`, whatever device the calling thread is on at `ovstream_start` time — `cudaSetDevice(N)` first to pin it). The consumer's process must select the same device (`cudaSetDevice`) before calling `ovstream_cudashm_client_create`, unless the two GPUs support peer access (the import enables it lazily). Match it out-of-band with the producer's `cuda_device`; after attaching, confirm with `client.producer_device` / `ovstream_cudashm_client_get_producer_device`, and on a mismatch the attach error names the producer's device. Cross-GPU CUDA IPC without peer access is not supported by the CUDA runtime.
- **Don't free the imported pointer.** `cudaFree` on a `frame.device_ptr` would be a use-after-free of the producer's GPU buffer. The consumer's lifetime contract is: read pixels (or copy them out), then call `wait_frame` again. Cleanup happens via `client.close()` / `ovstream_cudashm_client_destroy`, which calls `cudaIpcCloseMemHandle` on every imported handle.
